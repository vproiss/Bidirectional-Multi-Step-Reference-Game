...
## Project Execution
- The ``data_loader.py`` loads ``coco_captions`` dataset, extracts features with ResNet50, and saves ``train``, ``test`` and ``val`` datasets as ``{ds}.tfrecord`` files. You my skip this step – files are already saved in ``data`` folder for further usage.
